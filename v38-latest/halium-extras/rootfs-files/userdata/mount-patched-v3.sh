...
# v30 (Droidian), перенесено из параллельного (Ubuntu Touch) проекта на
# этом же устройстве, 2026-09-02: пропатченный camera.ko (тот же общий
# kernel source -- фиксит реальный logic bug в
# cam_sensor_wait_stream_on(), techpack/camera downstream driver,
# рапортовавший failure на genuinely-streaming frame_cnt, что вызывало
# ранний stream-off сразу после первого кадра). Модули ядра НЕ
# namespaced по контейнерам -- единая host-kernel-wide таблица, поэтому
# insmod ЗДЕСЬ, ДО старта контейнера (и значит до того, как Android's
# собственный modprobe/loader того же имени "camera" вообще попытается
# сработать) -- чьё имя "camera" зарегистрируется в ядре первым, тот и
# выигрывает; последующий Android-загрузчик того же модуля увидит
# "already loaded" и молча пропустит. Никакого container/mount-
# namespace-таймингового риска, в отличие от попыток биндить файл
# ВНУТРЬ контейнерного /vendor (тот путь физически проигрывает гонку --
# см. историю в параллельном проекте).
if [ -f "/opt/halium-lxc-bridge/camera-fixes/camera.ko" ]; then
    insmod /opt/halium-lxc-bridge/camera-fixes/camera.ko 2>/dev/null || true
fi
...
# CLAUDE_DEBUG 2026-09-05 (ФИКСЫ_ДЛЯ_В32.txt п.6.7): пропатченный
# libcameraservice.so (исключение для клиента hybris в handleAppOpMode)
# -- без него Android AppOps отзывает доступ к камере СРАЗУ после
# ПЕРВОГО кадра ("ровно один кадр и всё", поток не продолжается).
# `/system` внутри контейнера -- РЕАЛЬНЫЙ отдельный раздел
# (/dev/mapper/system, rw ext4), НЕ overlay/tmpfs -- bind-mount сюда
# работает так же надёжно, как для firmware/vndservicemanager ниже.
# Этот патч уже стоял ЖИВЬЁМ с 2026-09-04 10:17 (в тот момент камера
# впервые сняла настоящее фото), но был применён ТОЛЬКО ручной правкой
# файла на устройстве -- никогда не был застейджен как bind-mount,
# поэтому пропадал при каждой перезагрузке (найдено и застейджено
# только сейчас, при отладке живого потока в этой же сессии).
#
# CLAUDE_DEBUG 2026-09-05, ИСПРАВЛЕНО: живьём подтверждено через уже
# существовавший в этом файле диагностический блок (init-eloop-diag) --
# в момент выполнения lxc.hook.mount переменная ${R} (LXC_ROOTFS_MOUNT)
# указывает на /usr/lib/aarch64-linux-gnu/lxc/rootfs -- пустой служебный
# skeleton/tmpfs, НЕ на реальный смонтированный /system контейнера.
# Из-за этого и этот bind-mount, и (судя по всему) vndservicemanager
# ниже по файлу тихо проваливались (`|| true` глотал ошибку) с самого
# начала, несмотря на комментарий выше про "надёжно работает как для
# vndservicemanager". Реальный путь, где контейнер видит /system живьём
# -- статический /var/lib/lxc/android/rootfs (подтверждено вручную:
# mount --bind туда сработал и хеш файла внутри контейнера совпал с
# патченым). Используем его напрямую вместо ${R} для этого конкретного
# bind-mount'а -- остальной файл не трогаем.
echo "DIAG-CAMFIX: ls target 64: $(ls -la /var/lib/lxc/android/rootfs/system/lib64/libcameraservice.so 2>&1)"
mount --bind "/opt/halium-lxc-bridge/camera-fixes/libcameraservice.so.lib64" \
    "/var/lib/lxc/android/rootfs/system/lib64/libcameraservice.so"
echo "DIAG-CAMFIX: mount64 result rc=$?"
if [ -d "/var/lib/lxc/android/rootfs/system/lib" ]; then
    mount --bind "/opt/halium-lxc-bridge/camera-fixes/libcameraservice.so.lib32" \
        "/var/lib/lxc/android/rootfs/system/lib/libcameraservice.so"
    echo "DIAG-CAMFIX: mount32 result rc=$?"
fi
...
/usr/bin/mount --bind "${R}/system/system/build.prop" "${R}/system/build.prop" 2>/dev/null || true
...
# НАЙДЕНО 2026-07-28 (в3, перенесено в v25): контейнерный surfaceflinger --
# собственный composer3-клиент Android -- конкурирует за GPU/composer HAL
# с host-side компоситором (то же противостояние, из-за которого существует
...
# surfaceflinger падает на загрузке a660_zap.b02 ЕЩЁ ДО того как
# bounce-composer успевает сработать, роняя всю систему через hardware
# watchdog -- живьём воспроизведено на в3/v13, тот же init.rc/tot же
# GPU-код используется и здесь). surfaceflinger хосту не нужен вообще
# (host-компоситор рендерит напрямую через hwcomposer HAL) -- глушим его
# ТОЛЬКО внутри контейнера (bind-mount, не трогая тот же физический файл
# в baseline/system.img) тем же паттерном, что
# audioserver_HYBRIS_DISABLED/cameraserver_HYBRIS_DISABLED в этом же
# init.rc -- заменяем на no-op шелл-скрипт, exec завершается сразу,
# GPU/zap вообще не трогается.
...
echo "visfix tries used: ${VISFIX_TRIES}, result: $(grep -c 'com_android_art.visible' "${R}/linkerconfig/ld.config.txt" 2>/dev/null)"
...
# НАЙДЕНО 2026-07-22: sscrpcd (Sensors Secure RPC daemon, class early_hal,
# запускается ДО отключённого выше multihal HAL) сам по себе крутится в
# цикле "Error 0x200: open_dev (-1) failed for domain 0 (errno No such
# device)" каждые ~25мс -- нет нужной fastrpc-ноды (ADSP не проброшен
# через hybris/LXC). Раз потребитель (sensors-hal-multihal) уже отключён
# строкой выше, sscrpcd больше вообще никому не нужен -- отключаем его
# так же (class early_hal -> disabled), как и HAL. Живой bind-mount
# заглушки поверх работающего контейнера (через nsenter в mount-namespace
# контейнера) НЕ РАБОТАЕТ надёжно -- "No such file or directory" что с
# -m, что с -m -r (нижнеуровневая VFS-проблема с резолвом путей после
# смены namespace), плюс один живой pkill -9 sscrpcd совпал с ~30с
# пропаданием сети. Только этот boot-time патч, вступает в силу со
# следующей загрузки.
SSCRPCD_RC="${R}/vendor/etc/init/vendor.sensors.sscrpcd.rc"
[ -f "$SSCRPCD_RC" ] && patch_rc "$SSCRPCD_RC" \
    -e "s/^\(\s*\)class early_hal/\1class disabled/"
