...
# НАЙДЕНО 2026-09-03 (перенесено из в37): та же болячка tmpfs-/etc, что
# и везде в этом скрипте -- /etc/localtime и /etc/timezone никогда не
# были включены ни в один bind-mount, поэтому часовой пояс откатывался
# на UTC из образа при каждой полной перезагрузке. Восстанавливаем
# самым первым делом, до всего остального -- дёшево и ни от чего не
# зависит.
if [ -s /userdata/saved-timezone.txt ]; then
    _saved_tz=$(cat /userdata/saved-timezone.txt 2>/dev/null)
    if [ -n "$_saved_tz" ] && [ -e "/usr/share/zoneinfo/$_saved_tz" ]; then
        ln -sf "/usr/share/zoneinfo/$_saved_tz" /etc/localtime 2>/dev/null
        echo "$_saved_tz" > /etc/timezone 2>/dev/null
    fi
fi
...
# НАЙДЕНО 2026-07-27, ПЕРЕДЕЛАНО в v24: раньше здесь была отдельная
# ветка if/else на /userdata/PHOSH_FIRST_BOOT_OK, создававшая ОТДЕЛЬНЫЙ
# одноразовый 55с-таймер только на первую загрузку. Не сработало ни
# разу ни в v22, ни в v23 -- контейнер всё равно стартовал сразу же на
# первой загрузке, ломая SSH. Гейт первой загрузки теперь живёт ЦЕЛИКОМ
# в lxc-android-config.service.d/00-first-boot-gate.conf (ExecStartPre=
# /usr/libexec/halium-first-boot-delay.sh, sleep 50 + сам маркер) --
...
# вызывает systemctl start явно через 25с после boot, а не полагается
# на пассивный .wants pull-in.
# v28: этот юнит (и весь блок ниже, который его генерирует) СЕЙЧАС
# НЕ ИСПОЛЬЗУЕТСЯ -- update-binary маскирует
# halium-start-container-delayed.service/.timer НАВСЕГДА (не снимается
# никаким скриптом, в отличие от v27). Автостарт контейнера полностью
# переехал на отдельный постоянный halium-container-autostart.service/
# .timer (25с каждую загрузку, +20с дополнительно на самой первой,
# логика внутри одного скрипта) -- см. halium-extras/units/ и
# FIXES-V28-*.md. Блок ниже оставлен как есть (мёртвый код под маской)
# вместо удаления -- проще откатить, если единая схема окажется хуже
# двух отдельных механизмов на практике.
...
# НАЙДЕНО 2026-09-03 (перенесено из в37): сохранение часового пояса при
# его смене. Первая идея (systemd .path, следит за /etc/localtime через
# inotify) НЕ СРАБОТАЛА -- inotify-вотч на путь-символическую ссылку
# следует за её ТЕКУЩЕЙ целью, а смена цели ссылки (то, что делает
# timedatectl set-timezone) не модифицирует ни старый, ни новый
# файл-цель, событие никогда не приходит (подтверждено живьём: не
# сработало даже на ручной `ln -sf`). Вместо слежки за файлом --
# подписка на реальный источник события: D-Bus-сигнал PropertiesChanged
# от systemd-timedated (именно он меняет /etc/localtime при смене пояса
# через Настройки). `gdbus monitor` блокируется на чтении сокета и
# просыпается только на реальном сигнале -- никакого опроса/лишнего
# таймера в фоне.
cat > /etc/systemd/system/halium-timezone-changed.service <<'TZ_SVC_EOF'
[Unit]
Description=Watch systemd-timedated over D-Bus and save the timezone to /userdata whenever it changes (/etc is tmpfs, survives no reboot otherwise)
After=dbus.service

[Service]
Type=simple
ExecStart=/usr/libexec/halium-watch-timezone.sh
Restart=always
RestartSec=5
...
mkdir -p /etc/systemd/system/multi-user.target.wants
ln -sf ../halium-timezone-changed.service \
    /etc/systemd/system/multi-user.target.wants/halium-timezone-changed.service
...
systemctl start halium-timezone-changed.service 2>/dev/null || true
