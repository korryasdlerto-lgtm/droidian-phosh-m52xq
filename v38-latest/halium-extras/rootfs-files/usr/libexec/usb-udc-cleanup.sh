...
# v26: раньше был "exec ..." -- вывод настоящего конфигуратора уходил
# только в systemd/journal, а persistent journal на этом устройстве
# ненадёжен ("Device or resource busy" при ротации, не переживает
# ребут) -- живьём диагностировать невозможно. Ловим stdout+stderr в
# файл и построчно дублируем в /dev/kmsg (переживает ребут через
# pstore), сохраняя реальный exit-код через RC.
/usr/libexec/ubports-usb-moded-configurator > /tmp/ubports-configurator.out 2>&1
RC=$?
while IFS= read -r line; do
    echo "<6>usb-udc-configurator: $line" > /dev/kmsg 2>/dev/null
done < /tmp/ubports-configurator.out
log "real configurator exit status=$RC"
exit $RC
