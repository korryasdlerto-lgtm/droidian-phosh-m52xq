...
timeout 5 dbus-send --system --print-reply --dest=com.meego.usb_moded /com/meego/usb_moded com.meego.usb_moded.set_whitelisted_modes array:string:"developer_mode","mtp","mass_storage","charging_only" 2>&1

timeout 5 dbus-send --system --print-reply --dest=com.meego.usb_moded /com/meego/usb_moded com.meego.usb_moded.set_whitelisted string:developer_mode boolean:true

i=0
while [ $i -lt 6 ]; do
    result=$(timeout 5 dbus-send --system --print-reply --dest=com.meego.usb_moded /com/meego/usb_moded com.meego.usb_moded.set_mode string:developer_mode 2>&1)
...
# НАЙДЕНО И УБРАНО 2026-09-02: три постоянных фоновых захвата,
# добавленные здесь 2026-07-21 для ОДНОРАЗОВОГО живого расследования
# ~3-секундного дрейфа rndis.usb0 -> mass_storage.usb0 (udevadm
# monitor, dmesg -w, dbus-monitor --system -- полный системный
# D-Bus трафик), забыли убрать после того, как расследование
# завершилось. На живом устройстве обнаружено: /userdata/
# usb-dbus-monitor.log вырос до ~2ГБ всего за 26 минут одной
# загрузки (dbus-monitor --system пишет ВЕСЬ системный трафик),
# плюс dmesg -w (90МБ) и udevadm monitor (7.7МБ) -- три постоянных
# firehose-потока на каждой загрузке, весь аптайм, без остановки.
# Это была основная причина постоянного нагрева и быстрого разряда
# батареи (PID соответствующего процесса `sh` показывал 23%+ CPU
# непрерывно; ни suspend, ни низкое энергопотребление невозможны,
# пока это крутится). Диагностика 21 июля закончена, эти три блока
# были чисто временным инструментом -- удалены полностью. Если
# понадобится повторить то расследование, вставить обратно вручную
# на время одной live-сессии, не оставлять в постоянном скрипте.
...
while [ $i -lt 3600 ]; do
    ...
    case "$c1" in
        ...
        *)
            ...
            timeout 5 dbus-send --system --print-reply --dest=com.meego.usb_moded /com/meego/usb_moded com.meego.usb_moded.set_whitelisted string:developer_mode boolean:true >&4 2>&1
            timeout 5 dbus-send --system --print-reply --dest=com.meego.usb_moded /com/meego/usb_moded com.meego.usb_moded.set_mode string:developer_mode >&4 2>&1
            ;;
    esac
