...
# FOUND 2026-07-21: the "on nonencrypted" sed fix (removes the ONLY
# trigger for `class_start main`/`class_start late_start` -- zygote,
# system_server -- see pre-start.sh for the full history) lives in
# pre-start.sh, but pre-start.sh runs via lxc.hook.pre-start, which
# fires BEFORE this mount.sh (lxc.hook.mount) and before any of the
# config's own lxc.mount.entry lines are applied. At that point
# "${R}/system/etc/init/hw/init.rc" isn't real content yet (confirmed
# live: after a container ran with the pre-start.sh fix in place,
# `lxc-attach -n android -- grep "on nonencrypted" /system/etc/init/hw/
# init.rc` still found the line -- the sed silently no-opped on a
# not-yet-populated path). Right here, AFTER the SHALLOWFIX loop above
# (which is what actually makes "${R}/system/etc" resolve to real
# content), the file is genuinely readable/writable. Re-apply the same
# edit here as the real, working copy -- leave pre-start.sh's attempt in
# place too (harmless no-op if this one already did the job first).
REAL_INIT_RC="${R}/system/etc/init/hw/init.rc"
if [ -f "$REAL_INIT_RC" ]; then
    /usr/bin/sed -i "/mount_all /d" "$REAL_INIT_RC"
    /usr/bin/sed -i "/swapon_all /d" "$REAL_INIT_RC"
    /usr/bin/sed -i "/on nonencrypted/d" "$REAL_INIT_RC"
    echo "on nonencrypted removal: $(grep -c 'on nonencrypted' "$REAL_INIT_RC" 2>/dev/null || echo 0) occurrences remain"
else
    echo "REAL_INIT_RC missing at $REAL_INIT_RC -- cannot patch on nonencrypted"
fi
...
# НАЙДЕНО 2026-09-02: см. подробный комментарий в mount-patched-v3.sh --
# реальный PID 1 контейнера резолвится через /init ->
# /system/system/bin/init (не /system/bin/init), поэтому этот
# bind-mount годами целился мимо и never actually applied. Пристиновый
# init из самого system.img содержит вкомпилированную блокировку
# запуска vendor.camera-provider-2-6.
mount --bind /userdata/init-halium-nolimits "${R}/system/system/bin/init" 2>/dev/null || true
