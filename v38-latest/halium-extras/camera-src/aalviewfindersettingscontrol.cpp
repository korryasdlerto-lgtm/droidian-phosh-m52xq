...
void AalViewfinderSettingsControl::setSize(const QSize &size)
{
    ...
    bool wasPreviewStarted = m_service->isPreviewStarted();
    if (wasPreviewStarted) {
...
QSize AalViewfinderSettingsControl::chooseOptimalSize(const QList<QSize> &sizes) const
{
    if (!sizes.empty()) {
        if (m_aspectRatio == 0) {
            // CLAUDE_DEBUG 2026-09-04 (ФИКСЫ_ДЛЯ_В32 п.1, "остаётся
            // нерешённым" (а)): раньше тут безусловно бралось sizes[1] --
            // на этом HAL список превью-размеров близок к разрешению
            // самого сенсора, и вторым по счёту оказался кадр 3280x2464
            // (32МБ/кадр в RGBA), который валил приложение по памяти.
            // Берём размер, ближайший по площади к разумной цели 1280x720,
            // а не второй по счёту.
            const long targetArea = 1280L * 720L;
            QSize best = sizes[0];
            long bestDiff = std::labs(long(best.width()) * best.height() - targetArea);
            for (const QSize &s : sizes) {
                long diff = std::labs(long(s.width()) * s.height() - targetArea);
                if (diff < bestDiff) {
                    bestDiff = diff;
                    best = s;
                }
