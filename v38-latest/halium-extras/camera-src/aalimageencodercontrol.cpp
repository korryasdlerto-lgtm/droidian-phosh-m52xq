...
{
    // CLAUDE_DEBUG 2026-09-05: `continuous` был Q_UNUSED -- никогда не
    // устанавливался, оставался мусором со стека вызывающей стороны.
    // QtMultimedia's Camera::firstFourThreeResolution (main.qml:228)
    // может опираться на этот флаг при выборе размера из списка --
    // непредсказуемое поведение на мусоре объясняет задокументированную
    // "нестабильность" (иногда работает, иногда даёт undefined). Список
    // всегда дискретный (конкретные QSize, не диапазон) -- continuous
    // должен быть false.
    if (continuous)
        *continuous = false;
    Q_UNUSED(settings);

    // FIXES-V32 2026-09-04: QML droidian-camera (main.qml, fnAspectRatio())
    // читает supportedResolutions ДО того, как камера реально подключена --
    // m_availableSizes в этот момент ещё пуст (заполняется только в init(),
    // т.е. после connectCamera()). Пустой список => firstFourThreeResolution
    // остаётся undefined => "Cannot assign [undefined] to QSize" =>
    // НЕОБРАБОТАННОЕ исключение обрывает весь Component.onCompleted, и
    // стоящий следующей строкой camera.start() НИКОГДА не вызывается --
    // камера просто не запускается, видоискатель чёрный. Подтверждено
    // живьём gdb-брейкпоинтами: startPreview() не вызывается ни разу.
    // Отдаём безопасный фолбэк (по одному размеру на 4:3 и 16:9), пока
    // настоящий список не пришёл от HAL -- QML доходит до camera.start(),
    // камера подключается, и все последующие вызовы уже отдают реальные
    // размеры.
    if (m_availableSizes.isEmpty()) {
        QList<QSize> fallback;
        fallback.append(QSize(1440, 1080)); // 4:3
        fallback.append(QSize(1920, 1080)); // 16:9
        return fallback;
    }
...
{
    // CLAUDE_DEBUG 2026-09-05: тот же фикс, что и в supportedResolutions()
    // выше -- continuous не должен оставаться мусором со стека.
    if (continuous)
        *continuous = false;
    Q_UNUSED(settings);
