...
void AudioCapture::run()
{
    ...
    fprintf(stderr, "CLAUDE_DEBUG: AudioCapture::run() loop exited (bytesRead=%d bytesWritten=%d flagExit=%d)\n",
            (int)bytesRead, (int)bytesWritten, (int)m_flagExit);

    // Make sure that Pulse stops reading the microphone when recording stops
    if (m_paStream != NULL)
    {
        fprintf(stderr, "CLAUDE_DEBUG: AudioCapture::run() calling pa_simple_free()\n");
        pa_simple_free(m_paStream);
        m_paStream = NULL;
        fprintf(stderr, "CLAUDE_DEBUG: AudioCapture::run() pa_simple_free() returned\n");
    }
    fprintf(stderr, "CLAUDE_DEBUG: AudioCapture::run() returning\n");
}
...
bool AudioCapture::setupPipe()
{
    ...
    // 2026-08-29: opening a FIFO O_WRONLY blocks until a reader (the Android
    // audio HAL feeding AudioFlinger's real AudioRecord/RecordThread) opens
    // it too. If that reader is slow, this thread wedges here -- and since
    // deleteAudioCapture() later does m_audioCaptureThread.wait() from the
    // UI thread on ANY recorder-setup failure, that freezes the whole app.
    // Tried O_RDWR (never blocks on a FIFO per POSIX) but that changes the
    // pipe's reader/writer-count semantics enough that the Android-side
    // audio HAL reader started SIGABRTing (AudioFlinger's AudioIn_N thread) --
    // it apparently depends on a real, single, O_WRONLY-only writer. Instead:
    // poll with O_NONBLOCK (open() fails ENXIO with no reader present yet,
    // per POSIX FIFO semantics) so stopCapture()'s m_flagExit is checked
    // regularly, then drop back to a normal blocking fd once a reader is
    // actually there -- identical end state to the original O_WRONLY open.
    while (!m_flagExit)
    {
        m_audioPipe = open("/dev/socket/micshm", O_WRONLY | O_NONBLOCK);
        if (m_audioPipe >= 0)
            break;
        if (errno != ENXIO)
        {
    ...
    if (m_audioPipe < 0)
    {
        // stopCapture() was called before a reader ever showed up
        return false;
    }
    int pipeFlags = fcntl(m_audioPipe, F_GETFL, 0);
    fcntl(m_audioPipe, F_SETFL, pipeFlags & ~O_NONBLOCK);
