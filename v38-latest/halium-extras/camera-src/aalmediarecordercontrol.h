...
private:
    ...
    // 2026-08-30: runs fn() on a worker thread with a bounded wait (still
    // pumps UI events via a nested QEventLoop) -- used to guard binder
    // calls into cameraserver that can hang forever if the vendor camera
    // HAL has crashed. See stopRecording()/deleteRecorder() for why.
    static void runWithTimeout(std::function<void()> fn, int timeoutMs, const char *what);
