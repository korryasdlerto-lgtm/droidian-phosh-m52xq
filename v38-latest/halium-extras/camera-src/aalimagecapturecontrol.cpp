...
{
    m_lastRequestId++;
    qWarning() << "CLAUDE_DEBUG: capture() called, m_ready=" << m_ready
               << "androidControl=" << (void*)m_service->androidControl();
    if (!m_ready || !m_service->androidControl()) {
        qWarning() << "CLAUDE_DEBUG: capture() bailing out, not ready";
    ...
    // 2026-08-30: confirmed live -- android_camera_set_rotation(), called
    // right before take_snapshot(), was itself completely unguarded and
    // left 4 threads stuck in binder_thread_read with zero log output
    // (take_snapshot()'s own guard never even got a chance to fire since
    // execution never reached it). Same synchronous-binder-call-into-a-
    // wedged-HAL class as everything else fixed tonight.
    {
        CameraControl *camera = m_service->androidControl();
        runHalCallWithTimeout([camera, rotation]() { android_camera_set_rotation(camera, rotation); },
                              3000, "android_camera_set_rotation()");
    }

    // 2026-08-30: android_camera_take_snapshot() makes a synchronous binder
    // call into cameraserver -- same wedged-HAL hang class already fixed
    // for the recording path (see AalMediaRecorderControl), never guarded
    // here before. Confirmed live: a photo attempt right after a recording
    // attempt had already hit the wedged HAL left this stuck in
    // binder_thread_read with no timeout at all.
    {
        CameraControl *camera = m_service->androidControl();
        runHalCallWithTimeout([camera]() { android_camera_take_snapshot(camera); },
                              3000, "android_camera_take_snapshot()");
    }
